...
def upload_file():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if file and allowed_file(file.filename):
                ...
                isTongue = picSegment.pictureProcess(os.path.join(app.config['UPLOAD_FOLDER'], filename))
                ...
                try:
                    ...
                    if isTongue==True:
                        rate = runModel3.getResult(os.path.join(app.config['UPLOAD_FOLDER'], filename).replace(".", "_mask."))#这里需要注意，模型2是rec，模型3是mask
                        # print(os.path.join(app.config['UPLOAD_FOLDER'], filename).replace(".", "_rec."))
                        # print(rate)
                        ...
                        if len(tags)!=0:
                            ...
                            writeTotalRate = writePredictRate#这句干嘛来着我忘了
    ...
    #     if weight2Dict[i+2]>=0.5:
    #         judgeFlag = False

    # print(weight2Dict)

    if weight2Dict=={} and isTongue==True:
        message = "服务器已宕机，请联系我们重启程序!"

    if isTongue==True and weight2Dict!={}:
